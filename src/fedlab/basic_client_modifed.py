...
class SGDSerialClientTrainerExt(SGDSerialClientTrainer):
    ...
    def local_process(self, payload, id_list):
        ...
        for id in id_list:
            ...
            if id == 0:
                pack = self.repair(pack, data_loader, input_to_rule_map)
            if self.rules != None:
                rule_sat_cnt = evaluate_rules(self.model, self.rules, data_loader)
                print(
                    f'after repair: client {id}: % of inputs satisfying rules {[float(cnt) / len(data_loader.dataset) for cnt in rule_sat_cnt]}')
    ...
    def repair(self, model_parameters, train_loader, input_to_rule_map):
        ...
        # print(self._model)
        # print(model_parameters.shape)
        self.set_model(model_parameters[0])
        # self._model.train()
        ...
        if (self.personalization):
            ...
            for data, target in train_loader:
                ...
                for i in range(len(data)):
                    if input_to_rule_map[i + total] == -1:
                        continue
                    rule = self.rules[input_to_rule_map[i + total]]
                    for id in range(len(rule[1])):
                        ...
                        for id, op, vsig, row in constraints_list:
                            constraint = gp.quicksum(
                                row[j] * gurobi_w[j * ncol + id] + global_b[id] for j in range(ncol))
                            if op == "<=":
                                gurobi_model.addConstr(constraint <= vsig)
                            else:
                                # gurobi hard to optimize >
                        ...
                        try:
                            ...
                            constraints_list.append([id, op, vsig, layer2_input[i]])
                        except:
                            not_satisfied += 1
                total += len(data)
