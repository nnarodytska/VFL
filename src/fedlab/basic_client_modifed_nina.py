...
class SGDSerialClientTrainerExt(SGDSerialClientTrainer):
    ...
    def setup_optim(self, epochs, batch_size, lr):
        ...
        self.lr = lr
        params = []
        if self.concept_representation == "linear" or  self.concept_representation  is None:
            layer_names = []
            ...
            with torch.no_grad():
                ...
                for data, target in train_loader:
                    ...
                    for c_idx, concept_output in enumerate(concept_outputs):
                        ...
                        if cnt == 0:
                            print(f"init c_idx {c_idx}")
                            print(f"init concept_output {concept_output}")
        ...
        for epoch in range(self.epochs):
            ...
            for data, target in train_loader:
                ...
                if (self.personalization):
                    ...
                    #     #print( name, module)    
                    #     if name in ["fc1", "fc2"]:
                    #         print(f"rounds {rounds}: {name} {module.bias}")
                                  
                    #print(self.concept_representation)                
                    ...
                    if rounds >= self.personalization_rounds:
                        ...
                        if self.concept_representation == None:
                            ...
                            for c_idx, concept_label in enumerate(concept_labels):
                                ...
                                if rounds <= 1:
                                    print(f"rounds {rounds} c_idx {c_idx} loss {self.criterion(concept_label, input_to_concept_labels[idx_strt:idx_strt+batch_size, c_idx])}")
                                    print(concept_label) 
                                    print(input_to_concept_labels[idx_strt:idx_strt+batch_size, c_idx])

                            loss = loss_base + self.sim_weight * loss_concept
                            #print(loss_base, self.sim_weight, loss_concept, self.sim_weight * loss_concept)
                            self.optimizer.zero_grad()
                            loss.backward()
                            self.optimizer.step()
