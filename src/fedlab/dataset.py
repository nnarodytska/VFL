...
"""
Code copied from https://github.com/vanderschaarlab/CARs/blob/main/src/cars/utils/dataset.py
"""
...
class CUBDataset(Dataset):
            ...
            if self.no_img:
                if self.n_class_attr == 3:
                    one_hot_attr_label = np.zeros(
                        (self.N_ATTRIBUTES, self.n_class_attr)
                    )
    ...
    def concept_name(self, concept_id: int) -> str:
        ...
        full_name = linecache.getline(
            str(attributes_path), self.attribute_map[concept_id] + 1
        )
    ...
    def get_concepts_subset(
        ...
        for concept_idx in concept_ids:
            positive_ids = random.sample(
                self.concept_example_ids(concept_idx), instance_per_concept
            )
            example_ids += positive_ids
            negative_ids = random.sample(
                self.concept_example_ids(concept_idx, False), instance_per_concept
            )
...
class SEERDataset(Dataset):
    def __init__(
        ...
        random_seed: int,
        train: bool,
        load_concept_labels: bool = False,
        ...
        ).exists():  # If the train-test split has not been performed yet
            expected_columns = [
                "Age at Diagnosis",
                "PSA Lab Value",
                "T Stage",
...
def generate_mnist_concept_dataset(
    ...
    positive_loader = torch.utils.data.DataLoader(
        dataset, batch_size=subset_size, sampler=SubsetRandomSampler(positive_idx)
    )
    negative_loader = torch.utils.data.DataLoader(
        dataset, batch_size=subset_size, sampler=SubsetRandomSampler(negative_idx)
    )
...
def generate_ecg_concept_dataset(
    concept_class: int, data_dir: Path, train: bool, subset_size: int, random_seed: int
) -> tuple:
    """
...
def generate_seer_concept_dataset(
    dataset: SEERDataset, concept_id: int, subset_size: int, random_seed: int
) -> tuple:
    torch.manual_seed(random_seed)
    positive_ids = []
    negative_ids = []
    for patient_id, (patient_data, patient_label, patient_concept) in enumerate(
        iter(dataset)
    ):
    ...
    X = torch.stack(
        [dataset[idx][0] for idx in positive_ids[:subset_size]]
        + [dataset[idx][0] for idx in negative_ids[:subset_size]]
    )
